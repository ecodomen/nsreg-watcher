...
class Nsreg2domainsSpider(scrapy.Spider):
    ...
    def parse_price_change(self, response):
        price_change = response.xpath(
            "/html/body/div/div[1]/section[1]/div/div/div/div/div[2]/div[2]/div/span/text()"
        ).get()
    ...
    def parse(self, response):
        price_reg = response.xpath(
            '//*[@id="app"]/div[1]/section[3]/div/div[1]/div[1]/a/div[2]/text()'
        ).get()
        price_reg = find_price_withoutre(price_reg)

        price_prolong = response.xpath(
            '//*[@id="app"]/div[1]/section[3]/div/div[1]/div[1]/a/div[4]/text()'
        ).get()
        ...
        yield scrapy.Request(
            "https://2domains.ru/domains/transfer", callback=self.parse_price_change
        )
