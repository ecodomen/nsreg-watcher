...
class Nsreg2domainsSpider(scrapy.Spider):
    ...
    start_urls = ['http://a100.ru/']

    def parse(self, response):
        pricereg = response.xpath('//*[@id="overlappable"]/div/div/div/div[1]/div/div/div[2]/div/p[1]/text()')
        if m := re.match(REGEX_PATTERN, str(pricereg)):
            pricereg = f'{float(pricereg)}'
            logging.info('pricereg = %s', pricereg)
        
        priceprolong = response.xpath('//*[@id="overlappable"]/div/div/div/div[2]/div/div/div[2]/div/p[1]/text()')
